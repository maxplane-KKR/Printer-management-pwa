...
import { getPrinterWebUrl } from '../features/printers/printer-links';
...
function PrinterIp({ ip }: { ip: string }) {
  const href = getPrinterWebUrl(ip);
  if (!href) return <small>{ip}</small>;
  return <a className="printer-ip-link" href={href} target="_blank" rel="noopener noreferrer">{ip}</a>;
}
