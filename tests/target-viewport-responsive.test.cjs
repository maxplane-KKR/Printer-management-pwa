...
(async () => {
  ...
  const cases = [
    ...
    { width: 1024, height: 1280, expectedLayout: 'compact', expectedNav: 'grid' },
  ...
  for (const viewport of cases) {
    ...
    if (viewport.expectedLayout === 'compact') {
      ...
      if (viewport.width === 384) assert.ok(listMetrics.rowCount >= 4, `384x824px mobile list must show at least four rows: ${JSON.stringify(listMetrics)}`);
