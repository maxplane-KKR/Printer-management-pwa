...
async function main() {
  ...
  assert.match(html, /id="sheet-sync-indicator"[^>]*role="status"/);
  assert.match(html, /local:\s*'ข้อมูลสำรองในเครื่อง'/);
  ...
  assert.equal(
    dispatchFetch('https://cdn.example/icon.png'),
    undefined,
    'service worker ต้องไม่ cache ทรัพยากรข้าม origin'
  );
  assert.equal(
    dispatchFetch('https://printer.example/private/report'),
    undefined,
